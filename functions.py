...
def get_config(file = '/home/pes2ug19cs020/BigData/project/hay/Current_Config.json'):
	os.chdir(os.getcwd())
...
def configure(config):
	if config:
		...
		if not os.path.exists('Default.json'):
			print('Default configuration unavailable, please input a configuration')
			return
		file = 'Default.json'
	j = get_config(file)
	
	if os.path.exists(j['dfs_setup_config']):
	...
	if not os.path.exists(npath):
		os.makedirs(npath)
	with open(os.path.join(npath,"namenode.txt"),'w') as _:
		pass
	...
	with open(os.path.join(nlogpath,"namenode_log.log"),'w') as _:
		pass
	...
	with open(os.path.join(ncpoints,"namenode_checkpoints.txt"),'w') as _:
		pass
	...
	with open(dfs_config,'w') as dfsc:
		json.dump(j,dfsc)
...
def runmapper(mapperFile,block):
	dirname = os.path.dirname(block)
	filename = os.path.basename(block)
	os.chdir(dirname)
	os.system('cat '+filename+' | python3 '+mapperFile)	
	
		
def mapReduce(file,mapperFile):
	config = get_config()
	with open(os.path.join(config['path_to_namenodes'],'namenode.txt'),'rb') as nfile:
		temp = pickle.load(nfile)
	directory, filename = os.path.split(file)
			...
			if files[0] == filename and files[1][:-1] == directory:
				for blocks in files[2]:
					...
					if not flag:
						print('Block is missing')
						return
					t = threading.Thread(target=runmapper,args=(mapperFile,reps))
					t.start()
