...
my_parser.add_argument('-test',required=False)
...
if args.test:
	functions.mapReduce(inp,mapper)
	
elif mapper:
	 #changes - exe is always a text file and also the name of the output?
	 #os.remove('output.txt')
	 if not config or not reducer or not inp or not out:
	 	print('Missing arguments')
	 else:	
		 os.system('python3 hdfs.py -load '+config+' skip')
		 os.system('python3 hdfs.py -test true -m '+mapper+' -i'+inp+' | sort -k 1,1 | python3 '+reducer+' > output.txt')
